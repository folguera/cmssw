...
std::ostream& operator<<(std::ostream& out, const Key& o) {
  out << "Key_" << std::setw(2) << o.theNumber << " group " << std::setw(2) << o.theGroup << ":" << o.theIndexInGroup
...
StubResult GoldenPatternBase::process1Layer1RefLayer(unsigned int iRefLayer,
  ...
  for (auto& stub : layerStubs) {
    ...
    int phiDist = this->myOmtfConfig->foldPhi(
        hitPhi - phiMean -
        phiRefHit);  //for standard omtf foldPhi is not needed, but if one processor works for full phi then it is
    ...
    phiDist =
        phiDist >>
        this->getDistPhiBitShift(
            iLayer,
            iRefLayer);  //N.B. >> works well also for negative numbers. NB2. if the shift is done here, it means that the phiMean in the xml should be the same as without shift
