...
def addL3ToHLT(process):
	...
	for PFModule in producers_by_type(process, 'EgammaHLTCaloTowerProducer'):
		if hasattr(PFModule, 'L1NonIsoCand'):
			if(PFModule.L1NonIsoCand==cms.InputTag("hltL3MuonCandidates")):
				PFModule.L1NonIsoCand=cms.InputTag("hltIterL3MuonCandidates")				

		if hasattr(PFModule, 'L1IsoCand'):
			if(PFModule.L1IsoCand==cms.InputTag("hltL3MuonCandidates")):
				PFModule.L1IsoCand=cms.InputTag("hltIterL3MuonCandidates")

	for PFModule in producers_by_type(process, 'MuonHLTHcalPFClusterIsolationProducer'):
		if hasattr(PFModule, 'recoCandidateProducer'):
			if (PFModule.recoCandidateProducer==cms.InputTag( "hltL3MuonCandidates" )):
				PFModule.recoCandidateProducer=cms.InputTag("hltIterL3MuonCandidates")
	...
	IterMasterMuonTrackingRegionBuilder.input = cms.InputTag( 'hltL2SelectorForL3IO')	#Switch off for IO Only
...
#            dRDiffMax  = cms.double( 0.01 ),
	...
	process.hltL3MuonsIterIO = cms.EDProducer( "L3MuonProducer",
	    ServiceParameters = cms.PSet(
	      Propagators = cms.untracked.vstring( 'hltESPSmartPropagatorAny',
	        'SteppingHelixPropagatorAny',
	        'hltESPSmartPropagator',
	...
	process.hltIterL3MuonsLinksCombination = cms.EDProducer( "L3TrackLinksCombiner",
	    labels = cms.VInputTag( 'hltL3MuonsIterL3OI','hltL3MuonsIterIO' )
	)
	...
	process.hltL3fL1sMu22Or25L1f0L2f10QL3Filtered50Q.CandTag = cms.InputTag(  "hltIterL3MuonCandidates" )
	process.hltL3fL1sMu22Or25L1f0L2f10QL3Filtered50Q.InputLinks = cms.InputTag(  "hltIterL3MuonsLinksCombination" )
###	FOR IO Only:
#	process.hltIterL3MuonCandidates.InputLinksObjects = cms.InputTag( "hltL3MuonsIterIO" )
#	process.hltIterL3MuonCandidates.InputObjects = cms.InputTag( "hltL3MuonsIterIO" )
#       process.hltL3fL1sMu16orMu25L1f0L2f16QL3Filtered50Q.InputLinks = cms.InputTag( "hltL3MuonsIterIO" )
...
#       process.hltL3fL1sMu22Or25L1f0L2f10QL3Filtered50Q.InputLinks = cms.InputTag( "hltL3MuonsIterL3OI" )
###	#############################################################
	...
	process.HLTL3muonTkCandidateSequence = cms.Sequence(
	 ...
	 process.hltL3MuonsIterIO 		      	#off for OI
	)
